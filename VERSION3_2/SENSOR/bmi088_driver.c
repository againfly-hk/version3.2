...
uint8_t BMI088_init(void)//初始化bmi088
{
...
void BMI088_read_gyro_who_am_i(void)//who am i read gyro
{
    uint8_t buf;
...
void BMI088_read_accel_who_am_i(void)//who am i read accel
{
...
void BMI088_temperature_read_over(uint8_t *rx_buf, fp32 *temperate)//read temperature 
{
...
void BMI088_accel_read_over(uint8_t *rx_buf, fp32 accel[3], fp32 *time)//processing data accel, and time 
{
    int16_t bmi088_raw_temp;
...
void BMI088_gyro_read_over(uint8_t *rx_buf, fp32 gyro[3])//processing data gyro
{
    int16_t bmi088_raw_temp;
    bmi088_raw_temp = (int16_t)((rx_buf[1]) << 8) | rx_buf[0];
...
void BMI088_read(fp32 gyro[3], fp32 accel[3], fp32 *temperate)//read gryo accel temperature
{
...
void BMI088_read_gyro(fp32 gyro[3], fp32 *temperate)//only read gyro
{
    uint8_t buf[8] = {0, 0, 0, 0, 0, 0};
    int16_t bmi088_raw_temp;
...
fp32 get_BMI088_temperate(void)//get temperature 
{
    uint8_t buf[2];
    fp32 temperate;
...
void get_BMI088_gyro(int16_t gyro[3])//get gyro
{
    uint8_t buf[6] = {0, 0, 0, 0, 0, 0};
...
void get_BMI088_accel(fp32 accel[3])//get accel 
{
    uint8_t buf[6] = {0, 0, 0, 0, 0, 0};
